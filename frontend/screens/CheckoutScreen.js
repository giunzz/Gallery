...
const CheckoutScreen = ({ route, navigation }) => {
    ...
    const { addItemToLibrary } = useContext(LibraryContext); // ✅ Access LibraryContext
    ...
    const handleCheckout = async () => {
        ...
        if (paymentSuccessful) {
            addItemToLibrary(item); // ✅ Add new item to LibraryContext
            navigation.navigate("Library"); // ✅ Navigate to Library
        } else {
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <TouchableOpacity 
                ...
                onPress={handleCheckout} // ✅ Checkout and insert item to library
            >
...
const styles = StyleSheet.create({
    ...
    artworkContainer: { flexDirection: "row", alignItems: "center", backgroundColor: "#FFFFFF", padding: 16, borderRadius: 15, marginTop: 20 },
    artImage: { width: 60, height: 60, borderRadius: 10, marginRight: 10 },
    artTitle: { fontSize: 16, fontWeight: "bold" },
    artist: { fontSize: 14, color: "gray" },
    price: { fontSize: 18, fontWeight: "bold", color: "#2A9D8F", marginLeft: "auto" },
    paymentTitle: { fontSize: 18, fontWeight: "bold", marginTop: 20 },
    paymentOption: { flexDirection: "row", alignItems: "center", paddingVertical: 12 },
    selectedPayment: { backgroundColor: "#D1F0E8", borderRadius: 10, paddingHorizontal: 10 },
    paymentIcon: { width: 40, height: 30, marginRight: 10 },
    paymentText: { fontSize: 16, flex: 1 },
    totalContainer: { flexDirection: "row", justifyContent: "space-between", marginTop: 20 },
    totalText: { fontSize: 18, fontWeight: "bold" },
    totalPrice: { fontSize: 18, fontWeight: "bold", color: "#2A9D8F" },
    taxText: { fontSize: 12, color: "gray", textAlign: "right" },
    checkoutButton: { paddingVertical: 12, borderRadius: 10, alignItems: "center", marginTop: 20 },
    checkoutText: { fontSize: 16, color: "#FFF", fontWeight: "bold" },
});
