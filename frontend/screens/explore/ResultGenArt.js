...
const ResultGenArt = ({ route, navigation }) => {
  ...
  const fetchGeneratedArt = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Error",
        error.message || "Failed to generate artwork. Please try again."
      );
    } finally {
      setLoading(false);
    }
  ...
  useLayoutEffect(() => {
    navigation.setOptions({
      headerLeft: () => (
        <TouchableOpacity
          onPress={() => navigation.navigate("GenerateArtScreen", { prompt })}>
          <Ionicons
            name="arrow-back"
            size={24}
            color="black"
    ...
    <View style={styles.container}>
      ...
      <View style={styles.buttonContainer}>
        <TouchableOpacity
          style={[
            styles.button,
            activeButton === "Library" && styles.activeButton,
          ]}
        ...
        <TouchableOpacity
          style={[
            styles.button,
            activeButton === "Draw" && styles.activeButton,
          ]}
        ...
        <TouchableOpacity
          style={[
            styles.button,
            activeButton === "Explore" && styles.activeButton,
          ]}
